...
export default function PurchaseModal({ dataset, onClose, onSuccess }: PurchaseModalProps) {
  ...
  const handlePurchase = async () => {
    ...
    const buyerAddress = currentWallet?.accounts?.[0]?.address || (currentWallet as any)?.wallet?.accounts?.[0]?.address || (currentWallet as any)?.address;
    if (!buyerAddress) {
    ...
    try {
      ...
      const hash = await purchaseDataset(dataset, currentWallet, currentAccount, buyerAddress);
      setTxHash(hash);
      setStatus('success');
  ...
  return (
    <div className="fixed inset-0 bg-black/70 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl transition-colors duration-200">
        {/* Header */}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-900 dark:text-white font-medium text-sm">{formatDate(dataset.uploadDate)}</p>
              </div>
            </div>
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                <div
                  className={`h-2 rounded-full transition-all duration-300 ${
                    dataset.truthScore >= 80 ? 'bg-green-500' : dataset.truthScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                  }`}
            ...
            <div className="pt-4 border-t border-gray-200 dark:border-gray-700 mt-4">
              ...
              <p className="text-gray-700 dark:text-gray-300 font-mono text-sm">{formatAddress(dataset.owner)}</p>
            </div>
          </div>

          {/* Price */}
          <div className="bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-600/20 dark:to-primary-700/20 rounded-lg p-6 border border-primary-200 dark:border-primary-500/50">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Price</p>
                <p className="text-3xl font-bold text-gray-900 dark:text-white">{formatPrice(dataset.price)}</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-600 dark:text-gray-400">Downloads</p>
                <p className="text-2xl font-bold text-primary-600 dark:text-primary-400">{dataset.downloads}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex space-x-4">
            <button
              onClick={onClose}
              className="flex-1 px-6 py-3 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-900 dark:text-white font-semibold rounded-lg transition-colors duration-200"
              disabled={purchasing}
            >
              Cancel
            </button>
            <button
              ...
              className="flex-1 px-6 py-3 bg-primary-600 hover:bg-primary-700 dark:bg-primary-700 dark:hover:bg-primary-800 disabled:bg-gray-300 dark:disabled:bg-gray-700 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-all duration-200 flex items-center justify-center space-x-2 shadow-md hover:shadow-lg disabled:shadow-none"
            >
              {purchasing ? (
                <>
